...
class GATConvEIG(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        el = (th.cat([feat, graph.ndata['eig'][:, 1:3].unsqueeze(1)], dim=-1) * self.attn_l).sum(dim=-1).unsqueeze(-1)
        er = (th.cat([feat, graph.ndata['eig'][:, 1:3].unsqueeze(1)], dim=-1) * self.attn_r).sum(dim=-1).unsqueeze(-1)
