...
class GATConvEIG(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn_l_eig = nn.Parameter(th.FloatTensor(size=(1, num_heads // 2, 2)))
        self.attn_r_eig = nn.Parameter(th.FloatTensor(size=(1, num_heads // 2, 2)))
    ...
    def forward(self, graph, feat):
        ...
        print(feat_eig.shape)
        el_normal = (feat_normal * self.attn_l).sum(dim=-1).unsqueeze(-1)
        er_normal = (feat_normal * self.attn_r).sum(dim=-1).unsqueeze(-1)
        el_eig = (torch.abs(graph.ndata['eig'][:, 1:3]).unsqueeze(1).expand(-1, self._num_heads, -1) * self.attn_l_eig).sum(dim=-1).unsqueeze(-1)
        er_eig = (torch.abs(graph.ndata['eig'][:, 1:3]).unsqueeze(1).expand(-1, self._num_heads, -1) * self.attn_r_eig).sum(dim=-1).unsqueeze(-1)
        print(el_eig.shape)
        print(el_normal.shape)
        el = torch.cat([el_normal, el_eig], dim=1)
        er = torch.cat([er_normal, er_eig], dim=1)
        print(el.shape)
        print(er.shape)
