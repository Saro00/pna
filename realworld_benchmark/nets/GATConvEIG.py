...
class GATConvEIG(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        el_eig = (th.abs(graph.ndata['eig'][:, 1:3]).unsqueeze(1).expand(-1, self._num_heads, -1) * self.attn_l_eig).sum(dim=-1).unsqueeze(-1)
        er_eig = (th.abs(graph.ndata['eig'][:, 1:3]).unsqueeze(1).expand(-1, self._num_heads, -1) * self.attn_r_eig).sum(dim=-1).unsqueeze(-1)
        print(el_eig.shape)
