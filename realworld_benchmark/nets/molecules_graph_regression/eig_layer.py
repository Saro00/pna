...
class EIGLayer(nn.Module):
    ...
    def reduce_func(self, nodes):
        ...
        if self.NN_eig:
            ...
            print(torch.cat([eig_s[:][:][1], eig_d[:][:][1]], dim=-1).squeeze(-1).shape)

            print(self.eigfilt(torch.cat([eig_s[:][:][1], eig_d[:][:][1]], dim=-1).squeeze(-1)).unsqueeze(-1).shape)

            h = torch.cat([h, aggregate_NN(h, self.eigfilt(torch.cat([eig_s[:][:][1], eig_d[:][:][1]], dim=-1).squeeze(-1)).unsqueeze(-1))])
