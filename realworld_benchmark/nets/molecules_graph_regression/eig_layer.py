...
class EIGLayer(nn.Module):
    def __init__(self, in_features, out_features, dropout, graph_norm, batch_norm, aggregators, scalers, avg_d,
        ...
        if NN_eig:
            ...
            self.posttrans = MLP(in_size=(len(aggregators.split()) * len(scalers.split()) + 1) * in_features,
                             hidden_size=out_features,
                             out_size=out_features, layers=posttrans_layers, mid_activation='relu', last_activation='none')
        self.eigfilt = MLP(in_size=8, hidden_size=5, out_size=1, layers=3, mid_activation='relu', last_activation='none')
    ...
    def reduce_func(self, nodes):
        ...
        D = h.shape[-2]
        if self.NN_eig:
            w = self.eigfilt(torch.cat([eig_s[:, :, 1].unsqueeze(-1), eig_s[:, :, 2].unsqueeze(-1), eig_s[:, :, 3].unsqueeze(-1), eig_s[:, :, 4].unsqueeze(-1), eig_d[:][:, :, 1].unsqueeze(-1), eig_d[:][:, :, 2].unsqueeze(-1), eig_d[:][:, :, 3].unsqueeze(-1), eig_d[:][:, :, 4].unsqueeze(-1)], dim=-1))
            e = aggregate_NN(h, w)
