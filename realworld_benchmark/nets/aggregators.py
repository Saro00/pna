...
def aggregate_eig(self, h, eig_s, eig_d, eig_idx):
    ...
    h_mod = torch.mul(h, self.bias + (torch.abs(eig_s[:, :, eig_idx] - eig_d[:, :, eig_idx])/(torch.sum(torch.abs(eig_s[:, :, eig_idx] - eig_d[:, :, eig_idx]), dim=1, keepdim=True) + EPS)).unsqueeze(-1))
    return torch.sum(h_mod, dim=1)

def aggregate_eig_bis(self, h, eig_s, eig_d, eig_idx):
    #check right unsqueeze...
    h_mod = torch.mul(h, torch.mul(torch.sign(eig_s[:, :, eig_idx]).unsqueeze(-1),
